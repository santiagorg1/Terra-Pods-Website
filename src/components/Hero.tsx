...
const headline = ["Architectural pods,", null, "composed."]; // null = where italic emphasis sits
  ...
  return (
    ...
    >
      ...
      <motion.div aria-hidden style={{ y: yBg }} className="pointer-events-none absolute inset-0">
        ...
        <div className="absolute left-1/2 top-[30%] h-[90vh] w-[90vh] -translate-x-1/2 rounded-full bg-accent/[0.06] blur-[100px]" />
        <div className="absolute right-[10%] top-[55%] h-[40vh] w-[40vh] rounded-full bg-ember/[0.08] blur-[80px]" />
        {/* Light leak */}
      ...
      >
        ...
        >
          Terra Pods · 2028 Catalog
        </motion.span>

        <h1 className="display-1 mt-10 max-w-[14ch]">
          <motion.span
          ...
          <motion.span
            ...
            className="block"
          >
            <span className="display-italic text-gold">quietly</span>{" "}
            <span className="text-white/95">composed.</span>
          </motion.span>
          {/* keep ref to avoid lint */}
          <span className="hidden">{headline.length}</span>
        </h1>
        ...
        <motion.p
          ...
          className="lead-prose mt-12 max-w-xl text-balance text-ink-200"
        >
          Ten series. Twenty-six models. From a sixteen-square-metre studio to
          a forty-five-square-metre flagship — engineered in our factory and
          delivered complete on a single truck.
        </motion.p>
      ...
      <motion.div
        ...
        className="relative mb-12 flex flex-col items-center gap-3 text-[10px] uppercase tracking-[0.32em] text-ink-400"
      >
        <span>Scroll</span>
        <span className="block h-12 w-px bg-gradient-to-b from-accent/60 to-transparent" />
      </motion.div>
    </section>
  );
}
